...
学习路径：https://www.bilibili.com/video/BV19K4y1L7MT?p=4&spm_id_from=pageDriver

SpringBoot笔记文档：https://www.yuque.com/atguigu/springboot/rmxq85
...
@ResponseBody写在类上，代表该类中的每一个方法返回的数据都是写给浏览器的，而不是跳转到某个页面
----------------------------------------------------------------------------------------------------- @RestController
public class HelloController {
...
@ResponseBody注解包含了@ResponseBody|@Controller注解
-----------------------------------------------------------------------------------------------------
...
在pom.xml文件中，导入build插件，将项目直接打包成一个可执行的jar包或者war包
...
Spring Boot的版本仲裁中心 会自动导入对应的版本，不需要我们自己导入依赖，不用在项目的pom.xml里的dependencies里面管理的依赖自己声明
...
**Spring-boot-started-web的所有依赖**

<img src="https://gitee.com/pudding-bear-fithnis/picture-sourse/raw/master/SpringBoot/20210603154930.png" style="zoom:50%;" />
...
- - 主程序所在包及其下面的所有子包里面的组件都会被默认扫描进来
  ...
  程序扫描的时候SpringBoot自动会扫描主程序myapplication.java所在的包，以及myapplication的子包
  ```

  - 无需以前的包扫描配置
  - 想要改变扫描路径，@SpringBootApplication(scanBasePackages=**"com.atguigu"**)
  - 或者@ComponentScan 指定扫描路径（需将@SpringBootApplication注解替换为@SpringBootConfiguration
...
@EnableAutoConfiguration
@ComponentScan("com.atguigu.boot") //指定被扫描到的路径
...
- 按需加载所有自动配置项
  ...
  - SpringBoot所有的自动配置功能都在 **spring-boot-autoconfigure** 包里面
    ...
    <img src="https://gitee.com/pudding-bear-fithnis/picture-sourse/raw/master/SpringBoot/20210603161356.png" style="zoom:50%;" />
...
Spring Boot在2.2.0版本（依赖于Spring 5.2.0）起就把它的所有的自动配置类的此属性默认为true，
@Configuration(proxyBeanMethods = false)，可以提高Spring启动速度
```
 ...
 *      Lite(proxyBeanMethods = false)【每个@Bean方法被调用多少次返回的组件都是新创建的Bean】
 *      组件依赖必须使用Full模式默认。其他默认是否Lite模式
 */
//proxyBeanMethods = false: 表示为MyConfig对外没有依赖关系    
@Configuration(proxyBeanMethods = false) //告诉SpringBoot这是一个配置类 == 配置文件
public class MyConfig {

    /**
     * Full(proxyBeanMethods = true)模式:外部无论对配置类中的这个组件注册方法调用多少次获取的都是之前注册容器中的单实例对象
     * @return
     */
...
public class MainApplication {
    ...
    public static void main(String[] args) {
        ...
        //如果@Configuration(proxyBeanMethods = true)代理对象调用方法，SpringBoot总会先检查这个组件是否在容           器中有，有这个组件仍然返回原来的bean组件
        //保持组件单实例
...
<img src="https://gitee.com/pudding-bear-fithnis/picture-sourse/raw/master/SpringBoot/20210603185029.png" style="zoom:50%;" />
...
`@ImportResource("classpath:beans.xml")导入Spring的xml配置文件，将组件注册到容器中`
...
`@ConfigurationProperties(prefix = "mycar"):类中的每一个属性跟配置文件中的哪个前缀下的属性值一一绑定`
...
`在配置类中加上@EnableConfigurationProperties，从而指定加入到容器中的组件,若要引用第三方类库的属性，@EnableConfigurationProperties可以将第三方类加入到容器中，@Component不能加在第三方类库上`
...
**@Configuration**:配置类上，来标注这个注解(代理bean的方法)，代表当前是一个配置类；
...
`不需要组件依赖时使用Lite模式，即不需要创建新的组件时使用Lite,需要依赖组件就是要创建新的组件时，使用Full模式来保证取得的组件为ioc中的同一组件，而这两个模式在getBean时都是从ioc容器中拿的是同一组件`
...
以前需要自动配置的东西，现在Spring Boot帮我们自动配置；@EnableAutoConfiguration告诉SpringBoot开启自动
配置功能；这样自动配置才能生效。 
...
**@AutoConfigurationPackage**:自动配置包，自动导入？
...
//将指定的一个包下的所有组件导入进来？MainApplication(主程序) 所在包下的组件全部导入进来。 				
```

`AnnotationMetadata metadata:是标注在com.tencent.boot.MainApplication上`

<img src="https://gitee.com/pudding-bear-fithnis/picture-sourse/raw/master/SpringBoot/20210603195056.png" style="zoom:50%;" />

`new PackageImports(metadata).getPackageNames().toArray(new String[0]):将标注了@SpringBootApplication的主程序得到该程序的包名集合Collections，并转为String数组`

![](https://gitee.com/pudding-bear-fithnis/picture-sourse/raw/master/SpringBoot/20210603195719.png)
...
![](https://gitee.com/pudding-bear-fithnis/picture-sourse/raw/master/SpringBoot/20210531154653.png)

------

得出结论：**@AutoConfigurationPackage**是将主程序MainApplication所在的包的所有组件导入到容器中

------

**@Import({Registrar.class})**：底层注解，给容器导入组件；

`@Import({Registrar.class})需要写在能在IOC容器中存在的组件上，`默认组件的名字为全类名

```java
@Import({StaticMarkerBinder.class, DBHelper.class}) //给IOC容器中导入{StaticMarkerBinder,DBHelper}组件
@Configuration(proxyBeanMethods = true) //告诉SpringBoot这是一个配置类 == 配置文件
public class MyConfig {
    /**
     * @Description:给容器添加组件
     * 1.以方法名作为组件id,返回值类型就是组件类型返回的值,就是组件在容器中的实例
     * 2.(proxyBeanMethods = true)外部无论对配置类中的这个组件注册方法调用多少次获取的都是之前注册容器中的单实例对象
     */
    @Bean
    public User user01(){
        return new User("Jony",18);
    }
    @Bean
    public User userBean(){
        User user = new User("Jony", 18);
        //user组件依赖了Pet组件
...
//@Import({StaticMarkerBinder.class, DBHelper.class})给容器中自动创建出这两个类型的组件,默认组件的名字为全类名
```
...
- ![](https://gitee.com/pudding-bear-fithnis/picture-sourse/raw/master/SpringBoot/20210531155743.png) 
...
<img src="https://gitee.com/pudding-bear-fithnis/picture-sourse/raw/master/SpringBoot/20210531161720.png" style="zoom: 67%;" />

Spring Boot在启动的时候从**spring-boot-autoconfigure**包的路径下的**META-INF/spring.factorys**中获取的EnableAutoConfiguration指定的值；

将这些值作为自动配置类导入到容器中，自动配置就生效了。 ![2.factories](E:\工作文档\SpringBoot\images\2.factories.jpg)

![](https://gitee.com/pudding-bear-fithnis/picture-sourse/raw/master/SpringBoot/20210603202314.png)
...
5、自动加载的自动配置类是按需注在容器中(条件装配Conditional),并不是加载多少就注册多少到容器中    
...
`这些加载到容器内的配置configuration不仅包括系统自定义的配置类，还包括自定义的配置文件类`

<img src="https://gitee.com/pudding-bear-fithnis/picture-sourse/raw/master/SpringBoot/20210531155743.png" style="zoom: 67%;" />
...
----------------------------DispatcherServletConfiguration.java--------------------------------------
		@Bean
		@ConditionalOnBean(MultipartResolver.class)//容器有这个组件
    	//但容器中的这个组件名不是multipartResolver
...
如何在配置文件中配置组件属性：https://docs.spring.io/spring-boot/docs/current/reference/html/application-properties.html#application-properties.core
...
[^tips]: 需要在idea安装lombok插件，idea中搜索安装lombok插件
...
```java
-------------------------------------简化JavaBean开发-------------------------------------------------
@Data  //Lombok注解，自动帮开发者生成get、set方法
@NoArgsConstructor //自动生成无参构造器
@AllArgsConstructor //自动生成全参构造器
@EqualsAndHashCode  //自动生成hashcode方法
public class User {
    private String name;
    private Integer age;
    private Pet pet;
...
![](https://gitee.com/pudding-bear-fithnis/picture-sourse/raw/master/SpringBoot/20210531221537.png)

**自动引入的依赖**

![](https://gitee.com/pudding-bear-fithnis/picture-sourse/raw/master/SpringBoot/20210531221631.png)

# SpringBoot2核心技术-核心功能

![](https://gitee.com/pudding-bear-fithnis/picture-sourse/raw/master/SpringBoot/20210603214016.png)
...
相关文献：https://mkyong.com/spring/spring-propertysources-example/
...
![](https://gitee.com/pudding-bear-fithnis/picture-sourse/raw/master/SpringBoot/SLF4J.png)
...
![](https://gitee.com/pudding-bear-fithnis/picture-sourse/raw/master/SpringBoot/SLF4JandLogback.png)
...
> https://blog.csdn.net/weixin_44467186/article/details/107876562
...
--------------------------------------配置类中自定义--------------------------------------------------
	...
	//自定义filter,将_method这个名字换成自己定义的名字
    @Bean
...
**`需要在配置文件中手动配置(prefix = "spring.mvc.hiddenmethod.filter", name = "enabled")`**
...
![](https://gitee.com/pudding-bear-fithnis/picture-sourse/raw/master/SpringBoot/20210604102548.png)
...
<img src="https://gitee.com/pudding-bear-fithnis/picture-sourse/raw/master/SpringBoot/20210604105226.png" style="zoom:67%;" />

**`requestMappingHandlerMapping:保存了所有@RequestMapping和handler的映射规则`**

<img src="https://gitee.com/pudding-bear-fithnis/picture-sourse/raw/master/SpringBoot/20210604110920.png" style="zoom:67%;" />
...
![](https://gitee.com/pudding-bear-fithnis/picture-sourse/raw/master/SpringBoot/20210605165600.png)
...
`使用教程:https://www.thymeleaf.org/doc/tutorials/3.0/usingthymeleaf.html#appendix-b-expression-utility-objects`
...
<h2>
    ...
    <!--@{/link}:显示数据内容,若访问路径加了前缀,会自动加上该前缀-->
    <a href="http://www.baidu.com" th:href="@{/link}">百度</a>
</h2>
</body>
</html>
...
public class LoginInterceptor implements HandlerInterceptor {
     ...
     * 也可以进行一些判断来决定请求是否要继续进行下去。该方法的返回至是 Boolean 类型，当它返回 false 时，表示请求结束，
     * 后续的 Interceptor 和 Controller 都不会再执行；当它返回为 true 时会继续调用下一个 Interceptor 的 preHandle 方法，
...
/**
 * @author 布丁熊.菲特尼斯
 * @create 2021-06-06 15:23
 * 相关文章：https://segmentfault.com/a/1190000020094661
 */
...
public class AdminWebConfig{
    ...
    /**
     * @Description: 在组件WebMvcConfigurer配置拦截器
...
![](https://gitee.com/pudding-bear-fithnis/picture-sourse/raw/master/SpringBoot/20210606225222.png)
...
<form role="form" th:action="@{/upload}" method="post" enctype="multipart/form-data">
                            ...
                            <!--单个文件上传-->
                            <div class="form-group">
                                <label for="exampleInputFile">HeadImages</label>
                                <input type="file" name="headerImg" id="exampleInputFile">
                                <p class="help-block">Example block-level help text here.</p>
                            </div>
                            <!--多文件上传-->
                            <div class="form-group">
                                <label for="exampleInputFile">LifeImage</label>
...
public class FormTestController {
    ...
    public String upload(@RequestParam("email") String email,
        ...
        if (!headerImg.isEmpty()){
            ...
            //保存到文件服务器,OSS服务器
            headerImg.transferTo(new File("C:\\IdeaProject\\FrameWorkProjects\\SpringBoot\\UpLoadsFiles\\" + filename));
        }
        if (photos.length>0){
            for (MultipartFile photo:photos){
                ...
                //先判断文件是否为空
                if (!photo.isEmpty()){
                    photo.transferTo(new File("C:\\IdeaProject\\FrameWorkProjects\\SpringBoot\\UpLoadsFiles\\" + fileName));
                }
            }
        }
        return "index";
...
@Configuration(proxyBeanMethods = false)
	...
	//基于{DataSource.class, XADataSource.class}在容器中不存在
...
public class MyDataSourceConfig {
    ...
    /**
     * @Description: 配置Druid数据库连接池
     * 数据源自动配置类中没有DataSource.class，@ConditionalOnMissingBean(DataSource.class)
     *  才会向容器中注入HikariDataSource
     *  若容器存在存在DataSource.class，是不会向容器注入Hikari.class
     */
...
public class MyDataSourceConfig {
    ...
    public ServletRegistrationBean statViewServlet(){
        ...
        registrationBean.addInitParameter("loginPassword","6933");
        return registrationBean;
    }
     ...
     * 开启Spring监控,需要在该配置类上加
	 *  @ConditionalOnProperty("spring.datasource.druid.aop-patterns")，
	 * 还需在application.yml配置aop-patterns
...
spring:
  datasource:
#Spring的aop监控配置(Spring监控AOP切入点,如x.y.z.service.*,配置多个英文逗号分隔)
    druid:
      aop-patterns: "com.tencent.admin.*"
#Consider renaming one of the beans or enabling overriding by setting spring.main.allow-bean-definition-overriding=true       
  main:
    allow-bean-definition-overriding: true
```

**`注意，如果在某些场景使用StatViewServlet无法不生效，在1.2.5版本之后，提供了Filter的配置方式，参见`**

[配置_StatViewFilter]: https://github.com/alibaba/druid/wiki/%E9%85%8D%E7%BD%AE_StatViewFilter

### 5、Druid Spring Boot Starter

**`通过starter整合数据源`**

[starter整合]: https://github.com/alibaba/druid/tree/master/druid-spring-boot-starter
...
![](https://gitee.com/pudding-bear-fithnis/picture-sourse/raw/master/SpringBoot/20210607220423.png)
...
spring:
  datasource:
    ...
    username: root
    password: 6933
#    context-path: /world #在访问路径上加上world前缀
#Spring监控配置
    druid:
      #打开Druid的监控统计功能、配置防火墙
      filters: stat,wall
      ...
      #web监控(采集web-jdbc关联监控)
      web-stat-filter:
        enabled: true
        url-pattern: /*
        exclusions: '*.js,*.gif,*.jpg,*.png,*.css,*.ico,/druid/*'

      #aop监控配置(Spring监控AOP切入点,如x.y.z.service.*,配置多个英文逗号分隔)
      aop-patterns: "com.tencent.admin.*"

      #配置filter相关属性
      filter:
        stat:
          slow-sql-millis: 1000 #设置慢查询的sql为1000毫秒
...
## 11、Servlets, Filters, and listeners

[官方文档]: https://docs.spring.io/spring-boot/docs/current/reference/html/features.html#features.developing-web-applications.embedded-container.servlets-filters-listeners
...
**`当使用一个内置的servlet容器时,你可以注册servlets、filters、和所有的listeners从servlet说明文档，也可以通过Spring beans或者扫描Servlet组件`**

**`1、基于注解并通过@ServletComponentScan扫描Servlet组件`**
...
**`以上基于注解的三大组件类，若要起效,需要在SpringBoot主程序配置@ServletComponentScan扫描所有的servlet，注册到容器中`**

```java
@ServletComponentScan(basePackages = "com.tencent.admin")
@SpringBootApplication
public class MainApplication {
...
**`2、通过ServletRegistrationBean, FilterRegistrationBean、ServletListenerRegistrationBean向容器注入三大组件 `**

```java
package com.tencent.admin.config;

import com.tencent.admin.interceptor.LoginInterceptor;
import com.tencent.admin.servlet.MyFilter;
import com.tencent.admin.servlet.MyServlet;
import com.tencent.admin.servlet.MyServletContextListener;
...
![](https://gitee.com/pudding-bear-fithnis/picture-sourse/raw/master/SpringBoot/20210607222446.png)
...
<!DOCTYPE mapper
        PUBLIC "-//mybatis.org//DTD Mapper 3.0//EN"
        "http://mybatis.org/dtd/mybatis-3-mapper.dtd">
<mapper namespace="com.tencent.admin.mapper.CityMapper">
...
![](https://gitee.com/pudding-bear-fithnis/picture-sourse/raw/master/SpringBoot/20210615155007.png)
...
![](https://gitee.com/pudding-bear-fithnis/picture-sourse/raw/master/SpringBoot/20210615160218.png)
...
![](https://gitee.com/pudding-bear-fithnis/picture-sourse/raw/master/SpringBoot/20210615202601.png)

#### 2、1.x与2.x的不同

![](https://gitee.com/pudding-bear-fithnis/picture-sourse/raw/master/SpringBoot/20210615202652.png)
